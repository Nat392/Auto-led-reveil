...
class ZenggeBulbControllerTest {
    ...
    @Test
    fun `applyScene - processus complet de connexion BLE et déconnexion après succès (Happy Path)`() = runTest {
        ...
        val result = ZenggeBulbController.applyScene(
            context = context, 
            macAddress = "AA:BB:CC:DD:EE:FF", 
            red = 255, green = 120, blue = 0, white = 0, 
    ...
    @Test
    fun `openSession - expire et interrompt proprement en cas de timeout Gatt Hardware`() = runTest {
        val context = mockk<Context>(relaxed = true)
        val bluetoothManager = mockk<BluetoothManager>()
        val bluetoothAdapter = mockk<BluetoothAdapter>()
        val bluetoothDevice = mockk<BluetoothDevice>(relaxed = true)
    ...
    @Test
    fun `openSession - retourne null sans side-effects pour une MAC Address absente ou invalide`() = runTest {
        val context = mockk<Context>(relaxed = true)
        val bluetoothManager = mockk<BluetoothManager>()
        val bluetoothAdapter = mockk<BluetoothAdapter>()
    ...
    @Test
    fun `applyScene - sécurise et neutralise une SecurityException remontee par Android`() = runTest {
        val context = mockk<Context>(relaxed = true)
        val bluetoothManager = mockk<BluetoothManager>()
        val bluetoothAdapter = mockk<BluetoothAdapter>()
        ...
        every { context.getSystemService(BluetoothManager::class.java) } returns bluetoothManager
        every { bluetoothManager.adapter } returns bluetoothAdapter
        every { bluetoothAdapter.getRemoteDevice(any<String>()) } throws SecurityException("Missing BLUETOOTH_CONNECT")

        val result = ZenggeBulbController.applyScene(context, "AA:BB:CC:DD:EE:FF", 255, 128, 0, 0, 100)
