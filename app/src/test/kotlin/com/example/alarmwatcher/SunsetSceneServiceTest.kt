...
class SunsetSceneServiceTest {
    ...
    @Test
    fun `stops immediately when the action does not target the sunset scene`() =
        runTest {
    ...
    @Test
    fun `stops gracefully when the zone key is invalid`() =
        runTest {
            val intent = mockk<Intent>(relaxed = true)
    ...
    @Test
    fun `reports and stops when foreground startup is denied`() =
        runTest {
    ...
    @Test
    fun `applies the sunset scene with the configured bureau zone`() =
        runTest {
            val bureau =
                configuredZone(
                    label = "Bureau",
    ...
    @Test
    fun `returns false when the configured zone is missing`() =
        runTest {
